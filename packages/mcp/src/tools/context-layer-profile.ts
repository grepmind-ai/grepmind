...
export async function requireContextLayerSubagentProfile(
  ...
  if (!hasGrepmindMcp(profileRaw) && !hasGrepmindMcp(workspaceConfigRaw)) {
    throw new ContextLayerError(
      ...
      `Codex subagent profile is present at ${profilePath}, but Grepmind MCP was not found in the profile or ${workspaceConfigPath}. Configure the grepmind-context-layer-subagent profile or project .codex/config.toml so the research subagent can call code_search.`,
    );
  }
...
export async function verifyContextLayerSubagentProfile(
  ...
  if (!mcpList.ok) {
    throw new ContextLayerError(
      'CODEX_SUBAGENT_PROFILE_MISSING',
      `Codex subagent profile ${CONTEXT_LAYER_SUBAGENT_PROFILE} could not be checked with "codex mcp list --json". Configure the profile and project .codex/config.toml so the research subagent can call code_search. ${mcpList.message}`,
    );
  }
  ...
  if (!hasEnabledGrepmindServer(mcpList.output)) {
    throw new ContextLayerError(
      'CODEX_SUBAGENT_PROFILE_MISSING',
      `Codex subagent profile ${CONTEXT_LAYER_SUBAGENT_PROFILE} does not expose an enabled Grepmind MCP server in ${workspacePath}. Configure the profile or project .codex/config.toml so the research subagent can call code_search.`,
    );
  }
