...
export async function runCodexSubagent(
  ...
  const runDir = await createRunDir(input.dataDir);
...
async function createRunDir(dataDir: string): Promise<string> {
  const root = path.join(dataDir || os.tmpdir(), 'context-layer');
...
async function readLastMessage(
  ...
  return normalizeContextPackMarkdown(raw, context);
}
...
function truncateContextPack(
  ...
  return summarizeContextPackForLimit({
    contextPackMarkdown,
    maxOutputBytes,
    fullContextPackPath: outputPath,
    runtimeDurationMs,
  });
}
