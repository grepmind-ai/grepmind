...
- Codex config generated by `grepmind init --codex` uses
  `tool_timeout_sec = 375` by default so Grepmind's prompt-refiner plus
  research `context_layer` flow can complete or return its own timeout.
